		...
		if (index < joinSize) {
			...
			for (Object joinItem : join.getTarget()) {
				...
				if (OperatorHandle.isTrue(apply(join.getCondition(), variables, EMPTY_PARAMETER))) {
					...
					// 如果后续关联不上且不是left join时，跳过
					// 如果后续可以关联上且当前不是left join 也跳过
					if (processOtherJoin(index + 1, container) || !join.isLeftJoin()) {
						break;
					}
				}
			}
			variables.setValue(join.getAliasIndex(), Collections.emptyMap());
			// 没关联上且不是left join 直接过滤这一条数据
			return !matched && !join.isLeftJoin();
		}
		return false;
	}
